...
enable_advanced_collection() {
    ...
    echo ""
    echo -e "  ${BOLD}1. diagnostics.enabled = true${NC}"
    echo -e "     启用诊断数据输出，包括详细的 Run 事件、工具执行计时等"
    echo ""
    echo -e "  ${BOLD}2. logging.level = debug${NC}"
    echo -e "     启用 debug 级别日志，记录 Gateway 内部事件流"
    echo -e "     日志文件: /tmp/openclaw/openclaw-YYYY-MM-DD.log"
    echo ""
    echo -e "  修改后需要重启 OpenClaw Gateway 使配置生效。"
    echo -e "  ${GREEN}Collector 停止时会自动还原原始配置并重启 Gateway。${NC}"
    echo ""
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
...
interactive_config() {
    ...
    echo ""
    echo "  Server 端承诺："
    echo -e "    • 数据仅存储在内存中，${BOLD}不做任何持久化${NC}"
...
events_summary = {
    ...
    "model_usage": {
        "total_input_tokens": session_total_input,
